...
export const MockInterviewPlaceholder = ({ onAwardXp }) => {
  const [isRecording, setIsRecording] = useState(false);
  const [sessionActive, setSessionActive] = useState(false);
  const [activeQuestionIndex, setActiveQuestionIndex] = useState(0);
  const [wpmCadence, setWpmCadence] = useState(145);
  const [waveformBars, setWaveformBars] = useState([30, 65, 45, 80, 50, 90, 40, 75, 60, 35, 85, 55, 70, 40]);
